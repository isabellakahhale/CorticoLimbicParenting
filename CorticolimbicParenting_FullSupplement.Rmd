...
interact_plot(YSR.Total_kidkid_model, pred = Childhood.Stress, modx = Pos.Parenting.Youth.Report,plot.points = TRUE,modx.values = c(-1.0,0,1.0),colors="Qual1",  line.thickness = 1.5,partial.residuals = TRUE,facet.modx = TRUE, modx.labels = c("-1 SD Pos. Parenting","Mean Pos. Parenting","+1 SD Pos. Parenting")) + 
  xlab("Childhood Stress") + 
  ylab("Behavioral Problems: Youth Report")
```

**Figure 1.**  The association between childhood stress and youth behavioral problems at three levels of positive parenting. Youth reporting higher (+1 SD of positive parenting, in green) are shown on the left side of the figure; youth with mean levels of positive parenting are shown in red, and youth with lower levels of positive parenting (-1 SD) are shown in purple. For all of these graphs, stress is on the horizontal axis and behavioral problems (scaled, not raw YSR values) are on the vertical axis.
...
knitr::include_graphics("/Users/isabellakahhale/Library/CloudStorage/OneDrive-UniversityofPittsburgh/Isabella/Research/HippoParentingPaper/Submissions/PNAS_submission/Submission2_R&R/Figure2.png")
```

**Figure 2.** The association between childhood stress and total hippocampal volume at three levels of positive parenting. Youth reporting higher (+1 SD of positive parenting, in green) are shown on the left side of the figure; youth with mean levels of positive parenting are shown in red, and youth with lower levels of positive parenting (-1SD) are shown in purple. For all of these graphs, stress is on the horizontal axis and hippocampal volume is on the vertical axis. In the right corner, a 3-D figure showing the hippocampus, our region of interest. The hippocampus is depicted in blue and is shown on a transparent rendering of the whole brain.
...
hippo_interactplot <- interact_plot(Total_hippocampus_model_kidkid_raw, pred = Childhood.StressZ, modx = Pos.Parenting.Youth.ReportZ,plot.points = TRUE,modx.values = c(-1,0,1),colors="Qual1",  line.thickness = 1.5,partial.residuals = TRUE,facet.modx = TRUE, modx.labels = c("-1 SD Pos. Parenting","Mean Pos. Parenting","+1 SD Pos. Parenting"))  + 
...
#ggplot2::ggsave("Hippo_InteractionPlot.jpeg",hippo_interactplot)
...
```{r print hippocampal interaction terms, echo = FALSE}
...
```{r hippocampus model comparisions, eval=TRUE, echo=FALSE}
...
r.test(n = 482, 
       Total_hippocampus_model_kidkid@beta[8], 
       Total_hippocampus_model_kidparent@beta[8],
       0.8635238)
```
...
```{r amygdala model comparisions}
# compare beta from total amyg kid-kid model to total-parent model
# accounting for the correlation between the correlations
r.test(n = 482, 
       Total_amygdala_model_kidkid@beta[8], 
       Total_amygdala_model_kidparent@beta[8],
       0.8635238)
```
...
As reflected in the output below, the correlations between stress and SES variables are quite low (ranging from r  = -0.0085 to -0.0314) and non-significant at the p < 0.05 level.

```{r corrs between SES and stress, eval = TRUE, include = TRUE}
...
```{r SES and parenting, eval = TRUE, include=TRUE}
...
We used the WISC  Full Scale Intelligence Quotient (FSIQ) score as an index of cognitive ability. We first considered the correlation between FSIQ and stress, finding that the two constructs were not correlated. 


```{r corrs between IQ and stress, eval = TRUE, include = TRUE, echo=FALSE}
...
This binary diagnosis variable (i.e., *Any.Psychopathology*) was added to our main model predicting total hippocampal volume from an interaction between stress and youth-reported positive parenting, along with our main covariate set. Upon including the *Any.Psychopathology* variable, the interaction between childhood stress and youth-reported positive parenting is no longer significant. Of note, there is a *large* drop in sample size for these analyses (n=226 vs. n=400-480 in other models) due to available data; this drop may explain non-significant p-values, rather than a lack of associations between variables.
...
```{r total hippocampus kid kid gamm, eval= TRUE, include = TRUE, echo = FALSE}
...
#### Youth-Report Parenting and Stress on Total Hippocampal Volume GAMM Linear Mixed Effects Model Output
&nbsp;

```{r total hippocampus kid kid gamm mer output, eval= TRUE, include = TRUE, echo = FALSE}
summary(total_hippocampus_gamm_model$mer)
```


#### Youth-Report Parenting and Stress on Total Hippocampal Volume GAMM General Additive Model Output
&nbsp;

```{r total hippocampus kid kid gamm gam output, eval= TRUE, include = TRUE, echo = FALSE}
...
```{r total hippocampus kid parent gamm, eval= TRUE, include = TRUE, echo = FALSE}
total_hippocampus_gamm_model <-gamm4(TotalHippocampalVolume ~ Scan.Quality +
                                       Sex + s(Age)+ Total.Intracranial.Vol + 
                                       Childhood.Stress * Pos.Parenting.Caregiver.Report,
                                     random=~(1|Site), 
...
#### Caregiver-Report Parenting and Stress on Total Hippocampal Volume GAMM Linear Mixed Effects Model Output
&nbsp;

```{r total hippocampus kid parent gamm MER output, eval= TRUE, include = TRUE, echo = FALSE}
summary(total_hippocampus_gamm_model$mer)
```

#### Caregiver-Report Parenting and Stress on Total Hippocampal Volume GAMM General Additive Model Output
&nbsp;

```{r total hippocampus kid parent gamm GAM output, eval= TRUE, include = TRUE, echo = FALSE}
summary(total_hippocampus_gamm_model$gam)
...
```{r total amyg kid kid gamm, echo = FALSE}
...
```{r total amyg kid parent gamm, echo = FALSE}
total_amygdala_gamm_model <-gamm4(TotalAmygdalaVolume ~ Scan.Quality + 
                                    Sex + s(Age)+ Total.Intracranial.Vol + 
                                    Childhood.Stress * Pos.Parenting.Caregiver.Report,
                                  random=~(1|Site), 
...
```{r nonlinear stress lmem prep, eval=TRUE, include=TRUE, echo = FALSE}
...
```{r total amyg kid kid nonlinear stress lmem, eval=TRUE, include=TRUE, echo = FALSE}
...
```{r left amyg kid kid nonlinear stress lmem, eval=TRUE, include=TRUE, echo = FALSE}
...
```{r right amyg kid kid nonlinear stress lmem, eval=TRUE, include=TRUE, echo = FALSE}
...
```{r total amyg kid kid nonlinear stress gmv lmem, eval=TRUE, include=TRUE, echo = FALSE}
...
```{r left amyg kid kid nonlinear stress gmv lmem, eval=TRUE, include=TRUE, echo = FALSE}
...
```{r right amyg kid kid nonlinear stress gmv lmem, eval=TRUE, include=TRUE, echo = FALSE}
...
# this doesn't work -- jusst assignes 1 to first 482. need it to be the same identifier
...
# Corrections for Multiple Comparisions: Main Models
...
pvals$adjusted_BH <-0 
pvals$adjusted_BH<- pvaladjust$BH_adjp
pvals$adjusted_Storey <-0 
pvals$adjusted_Storey<- pvaladjust$Storey_adjp
pvals$adjusted_BonEV <-0 
pvals$adjusted_BonEV<- pvaladjust$Bon_EV_adjp

#write.csv(pvals, "adjustedvals.csv")
```
